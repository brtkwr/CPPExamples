...
zRotationIncrement,HorizontalRotationIncrement,zRotation=0.0,HorizontalRotation=0.0,cosz,cosH,sinz,sinH,PI,supportRadius,
InitialButtonX[lastButton + 1],InitialButtonY[lastButton + 1],ButtonX[lastButton + 1],ButtonY[lastButton + 1];

int clickx,clicky,ButtonOn[lastButton + 1];

bool LeftMouseDown=0,RightMouseDown=0;

double SupportHeight,SupportHorizontalPosition;
...
void MyGraphics(double BackGroundRed,double BackGroundBlue,double BackGroundGreen)
{
	...
	InitialZoom=0.95*double(WindowHalfHeight)/supportRadius;
	Zoom=InitialZoom;
	...
	InitialButtonX[1] = - 0.95;
	InitialButtonY[1] = - 0.1;
	
	InitialButtonX[2] = - 0.95;
	InitialButtonY[2] = - 0.2;
...
void MouseDrag(int x, int y) 
{
	...
	{
		if(ButtonOn[0] == 1  || ButtonOn[1] == 1  || ButtonOn[2] == 1)
		{
			...
			if(ButtonOn[1] == 1)
			{
				...
				SupportHeight -= supportRadius * 0.001 * double(y - clicky);
			}
			if(ButtonOn[2] == 1)
			{
				ButtonX[2] = double(x) / double(WindowHalfWidth) - 1.0;
				ButtonY[2] = - (double(y) / double(WindowHalfHeight) - 1.0);
				SupportHorizontalPosition -= supportRadius * 0.001 * double(y - clicky);
			}
		}
		else
		{
