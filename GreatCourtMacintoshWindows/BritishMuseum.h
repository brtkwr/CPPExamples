...
int  NodeType[m + 1][n + 1],CoordinateDirection[3];
...
void SetupData(void)
{
	CoordinateDirection[0] = xDirection;
	CoordinateDirection[1] = yDirection;
	CoordinateDirection[2] = zDirection;
...
void CalculateAndDraw(void)
{
	...
	chris_vertices[     CoordinateDirection[0]] = + b;chris_vertices[     CoordinateDirection[1]] = - d;chris_vertices[     CoordinateDirection[2]] = 0.0;
	chris_vertices[ 3 + CoordinateDirection[0]] = + b;chris_vertices[ 3 + CoordinateDirection[1]] = + c;chris_vertices[ 3 + CoordinateDirection[2]] = 0.0;
	chris_vertices[ 6 + CoordinateDirection[0]] = - b;chris_vertices[ 6 + CoordinateDirection[1]] = + c;chris_vertices[ 6 + CoordinateDirection[2]] = 0.0;
	chris_vertices[ 9 + CoordinateDirection[0]] = - b;chris_vertices[ 9 + CoordinateDirection[1]] = - d;chris_vertices[ 9 + CoordinateDirection[2]] = 0.0;
	chris_vertices[12 + CoordinateDirection[0]] = + b;chris_vertices[12 + CoordinateDirection[1]] = - d;chris_vertices[12 + CoordinateDirection[2]] = 0.0;
	...
	for(int itry = 0;itry <= m + 1;itry ++)
	{
		...
		for(int xyz = 0; xyz <= 2;xyz ++)chris_vertices[3 * itry + CoordinateDirection[xyz]] = ReadingRoomPosition[i][xyz];
	...
	for(int j = 0;j <= n;j ++)
	{
		...
		for(int itry = 0;itry <= m + start;itry ++)
		{
			...
			if(NodeType[i][j] >= 0)
			{
				...
				myVertex ++;
				if(LineDrawStep == 0)AddVertex(myVertex,i,j,GridRed,GridGreen,GridBlue,GridEmpahsisAlpha);
				else AddVertex(myVertex,i,j,GridRed,GridGreen,GridBlue,GridAlpha);
			}
			else
			{
				if(NowDrawing == 1)
	...
	for(int i = subdivision;i <= m + 1 - subdivision;i += 2 * subdivision)
	{
		...
		for(int j = n;j >= 0;j -= subdivision)
		{
			...
			{
				myVertex ++;
				AddVertex(myVertex,Actual_i,j,StructureRed,StructureGreen,StructureBlue,StructureAlpha);
			}
		}
		plotLinesOrPoints(1,myVertex);
	}
	...
	for(int i = 0;i <= m;i += subdivision)
	{
		...
		for(int j = n - LineStart;j >= 0;j -= 2 * subdivision)
		{
			...
			{
				myVertex ++;
				AddVertex(myVertex,i,j,StructureRed,StructureGreen,StructureBlue,StructureAlpha);
			}
			else
			{
				...
				{
					myVertex ++;
					AddVertex(myVertex,i,j + subdivision,StructureRed,StructureGreen,StructureBlue,StructureAlpha);
				}
			}
		}
		plotLinesOrPoints(1,myVertex);
		...
		for(int i = 0;i <= m;i += subdivision)
		{
			for(int j = n;j >= 0;j -= subdivision)
			{
				if(NodeType[i][j] == 0)
				{
					...
					{
						chris_vertices[CoordinateDirection[xyz]] = x[i][j][xyz];
						chris_vertices[CoordinateDirection[xyz] + 3] = x[i][j][xyz] + 0.5 * Normal[i][j][xyz];
					}
	...
	if(start == 1)
	{
		for(int CalculationLoop = 0;CalculationLoop <= LastCalculationLoop;CalculationLoop++)
		{
			if(MotionStartControl < 1.0)MotionStartControl += MotionStartControlIncrement;
...
void AddVertex(int myVertex,int i,int j,GLfloat Red,GLfloat Green,GLfloat Blue,GLfloat ColourAlpha)
{
	if(myVertex <= LastPossibleVertex)
	{
		for(int xyz = 0; xyz <= 2;xyz ++)
			chris_vertices[3 * myVertex + CoordinateDirection[xyz]] = (GLfloat)x[i][j][xyz];
		chris_colours[4 * myVertex + 0] = Red;
		chris_colours[4 * myVertex + 1] = Green;
		chris_colours[4 * myVertex + 2] = Blue;
		chris_colours[4 * myVertex + 3] = ColourAlpha;
	}
}
