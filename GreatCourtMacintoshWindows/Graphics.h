...
zRotationIncrement,HorizontalRotationIncrement,zRotation = 0.0,HorizontalRotation = 0.0,cosz,cosH,sinz,sinH,PI,
InitialButtonX[lastButton + 1],InitialButtonY[lastButton + 1],ButtonX[lastButton + 1],ButtonY[lastButton + 1];

int clickx,clicky,ButtonOn[lastButton + 1],start,freshStart;
...
void MyGraphics(double BackGroundRed,double BackGroundBlue,double BackGroundGreen)
{
	...
	InitialZoom = double(WindowHalfHeight) / 60.0;
	Zoom = InitialZoom;
	glutInitWindowPosition(0,0);
	glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE | GLUT_DEPTH);
	glutCreateWindow("British Museum Great Court");
	...
	{
		InitialButtonX[Button] = - 0.95;
		InitialButtonY[Button] = - 0.075 * (double)Button;
...
static void Key(unsigned char key, int x, int y)
{
	switch (key)
	{
		case ' ':if(start == 1)freshStart = 1;else start = 1;break;
		case 's':if(start == 1)start = 0;else start = 1;break;
...
void MouseClick(int button, int state, int x, int y) 
{
	...
	if ((button == GLUT_LEFT_BUTTON)&&(state == GLUT_DOWN)) {
		...
		clicky = y;
		LeftMouseDown = 1;
		if(ButtonOn[4] == 1)start = 1;
		if(ButtonOn[5] == 1)start = 0;
		if(ButtonOn[6] == 1)freshStart = 1;
	}
...
void MouseDrag(int x, int y) 
{
	...
	{
		if(ButtonOn[0] == 1 || ButtonOn[1] == 1 || ButtonOn[2] == 1 || ButtonOn[3] == 1)
		{
			...
			if(ButtonOn[1] == 1)
			{
				ButtonX[1] = double(x) / double(WindowHalfWidth) - 1.0;
				ButtonY[1] = - (double(y) / double(WindowHalfHeight) - 1.0);
				lambda -= 0.01 * double(y - clicky);
				if(lambda < 0.0)lambda = 0.0;
			}
			if(ButtonOn[2] == 1)
			{
				ButtonX[2] = double(x) / double(WindowHalfWidth) - 1.0;
				ButtonY[2] = - (double(y) / double(WindowHalfHeight) - 1.0);
				alpha -= 0.01 * double(y - clicky);
			}
			if(ButtonOn[3] == 1)
			{
				ButtonX[3] = double(x) / double(WindowHalfWidth) - 1.0;
				ButtonY[3] = - (double(y) / double(WindowHalfHeight) - 1.0);
				beta -= 0.01 * double(y - clicky);
			}
		}
		else
		{
