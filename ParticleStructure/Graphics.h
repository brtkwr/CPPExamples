...
int argc,clickx,clicky,FileNumber,ShowFibres,ShowParticles,DrawGrid,ZoomOrRotate = 0;
...
void SetUpGraphics(float BackGroundRed,float BackGroundGreen,float BackGroundBlue)
{
	...
	ShowFibres = 1;ShowParticles = 1;DrawGrid = 0;
...
static void Key(unsigned char key, int x, int y)
{
	switch (key)
	{
		...
		case 'f':ShowFibres ++;if(ShowFibres > 2) ShowFibres = 0;break;
		case 'p':ShowParticles ++;if(ShowParticles > 2) ShowParticles = 0;break;
...
#else
		if(ThisNumber == 0)strcat_s(FileName,zero);
		if(ThisNumber == 1)strcat_s(FileName,one);
		if(ThisNumber == 2)strcat_s(FileName,two);
		if(ThisNumber == 3)strcat_s(FileName,three);
		if(ThisNumber == 4)strcat_s(FileName,four);
		if(ThisNumber == 5)strcat_s(FileName,five);
		if(ThisNumber == 6)strcat_s(FileName,six);
		if(ThisNumber == 7)strcat_s(FileName,seven);
		if(ThisNumber == 8)strcat_s(FileName,eight);
		if(ThisNumber == 9)strcat_s(FileName,nine);
#endif
	}
#if defined(__APPLE__)
	strcat (FileName,".tga");
