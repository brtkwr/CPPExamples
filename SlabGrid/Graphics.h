...
void MyGraphics(double BackGroundRed,double BackGroundBlue,double BackGroundGreen)
{
	...
	InitialZoom=0.95*double(WindowHalfHeight)/(2.0 * a);
	Zoom=InitialZoom;
...
static void Key(unsigned char key, int x, int y)
{
	switch (key)
	{
		...
		case 'q':writeDXF();gluDeleteQuadric(quadObj);exit(0);
		case 'Q':writeDXF();gluDeleteQuadric(quadObj);exit(0);
		case '\033':writeDXF();gluDeleteQuadric(quadObj);exit(0);
	}
}
